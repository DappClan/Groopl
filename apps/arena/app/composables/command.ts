...
export type QueryResultItem = CommandQueryResultItem
...
export function provideGlobalCommands() {
  ...
  useCommand({
    ...
    icon: () => userSettings.value.preferences.grayscaleMode ? 'i-ri:contrast-2-line' : 'i-ri:contrast-line',

    onActivate() {
      togglePreferences('grayscaleMode')
    },
  })
  ...
  useCommand({
    ...
    visible: () => wallets.value.length > 1,
  ...
  useCommands(() => wallets.value.map(wallet => ({
    parent: 'wallet-switch',
    scope: 'Switch Wallet',

    visible: () => wallet.connection.accountId !== currentWallet.value?.connection.accountId,

    name: `Switch to ${wallet.connection.accountId}`,
    icon: 'i-ri:wallet-line',
  ...
  useCommand({
    ...
    visible: () => isWalletConnected.value,
