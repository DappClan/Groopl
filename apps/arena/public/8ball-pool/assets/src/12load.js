...
(loadState.init = function () {
    ...
    (e.loaderProgress = 0),
        ...
        (e.loaderCanvas = new Phaser.Group(game, game.stage, "loaderCanvas")),
        (e.loaderCanvas.alpha = 0),
        (e.loaderCanvas.x = game.width / 2),
        (e.loaderCanvas.y = game.height / 2),
        game.stage.addChild(e.loaderCanvas),
        (e.title = new Phaser.Sprite(game, 0, 0, "title")),
        e.loaderCanvas.addChild(e.title),
        (e.title.anchor = new Point(0.5, 0.5)),
        (e.rack = new Phaser.Sprite(game, 0, 0, "rack")),
        e.loaderCanvas.addChild(e.rack),
        (e.rack.anchor = new Point(0.5, 0.5)),
        (e.loaderBase = new Phaser.Sprite(game, 0, 800, "loaderBase")),
        (e.loaderBase.anchor = new Point(0.5, 0)),
        e.loaderCanvas.addChild(e.loaderBase),
        (e.loaderFill = new Phaser.Sprite(game, 0, 800, "loaderFill")),
        (e.loaderFill.anchor = new Point(0.5, 0)),
        e.loaderCanvas.addChild(e.loaderFill),
        (e.graphics = game.add.graphics(0, 0)),
        e.graphics.beginFill(16777215),
        (e.loaderFill.mask = e.graphics),
        (e.loaderHighlight = new Phaser.Sprite(game, 0, 80, "loaderHighlight")),
        (e.loaderHighlight.anchor = new Point(0.5, 0)),
        e.loaderCanvas.addChild(e.loaderHighlight),
        (e.progress = new Object()),
        (e.progress.val = 0),
        this.resizeGame();
    game.add.tween(e.loaderCanvas).to({ alpha: 1 }, 1e3, Phaser.Easing.Linear.None, !0, 500);
}),
    (loadState.resizeGame = function (e, s) {
        var t = this.loaderInfo;
        // Safety check: return early if loader isn't initialized yet
        if (!t || !t.loaderCanvas) return;
        ...
        "portrait" == window.famobi.getOrientation()
            ? (game.scale.setGameSize(1080, 1920), (t.title.y = (-game.height / 2) * 0.5))
            : (game.scale.setGameSize(1920, 1080), (t.title.y = (-game.height / 2) * 0.6));
        String(Math.abs(a - document.getElementById("mygame").clientHeight) / 2);
        (t.loaderCanvas.x = game.width / 2),
            (t.loaderCanvas.y = game.height / 2),
            (t.loaderBase.y = (0.6 * game.height) / 2),
            (t.loaderFill.y = (0.6 * game.height) / 2),
            (t.loaderHighlight.y = (0.6 * game.height) / 2),
            (t.rack.y = (game.height / 2) * 0.1),
            t.graphics.clear(),
            t.graphics.beginFill(16777215),
            t.graphics.drawRect(
                game.width / 2 - 187.5,
                game.height / 2 + t.loaderBase.y + 8,
                3.75 * t.loaderProgress,
                30
            );
    }),
    ...
    (loadState.preload = function () {
        ...
        let e = famobi.hasFeature("external_mute");
        this.load.image("panel4", e ? "assets/img/panel4e.png" : "assets/img/panel4d.png"),
    ...
    (loadState.updateProgressBar = function () {
        var e = this.loaderInfo;
        (e.loaderProgress = game.load.progress),
            e.graphics.clear(),
            e.graphics.beginFill(16777215),
            e.graphics.drawRect(
                game.width / 2 - 187.5,
                game.height / 2 + e.loaderBase.y + 8,
                3.75 * e.loaderProgress,
                30
            );
    }),
    ...
    (loadState.create = function () {
        var e = this.loaderInfo;
        function s() {
            // AUTO-START PVP MODE - Skip menu and go straight to game
            (projectInfo.levelComplete = !1);
            projectInfo.mode = 2; // 2 = Player vs Player
            projectInfo.tutorial = !1; // Skip tutorial
            
            // Initialize scores (normally done in menu)
            projectInfo.score = 0;
            try {
                var storedBestScore = window.famobi.localStorage.getItem("bestScore");
                projectInfo.bestScore = storedBestScore ? parseInt(storedBestScore) : 0;
            } catch (e) {
                projectInfo.bestScore = 0;
            }
            
            // Initialize AI rating (for display purposes, though not used in PvP)
            try {
                var storedAiRating = window.famobi.localStorage.getItem("aiRating");
                projectInfo.aiRating = storedAiRating ? parseInt(storedAiRating) : 1;
            } catch (e) {
