var Ball = function (t, s) {
    ...
    (this.ballRotation = new Array(1, 0, 0, 0)),
        ...
        (this.circRad = t),
        s <= 8
            ? ((this.ballType = 0), (h = "solidsSpriteSheet"))
            : ((this.ballType = 1), (h = "ballSpriteSheet" + String(s))),
        Phaser.Group.call(this, game, this, "ball"),
        (this.sprite = new Phaser.Sprite(game, 0, 0, h)),
        (this.sprite.width = 2 * this.circRad),
        (this.sprite.height = 2 * this.circRad),
        (this.sprite.anchor = new Phaser.Point(0.5, 0.5)),
        this.addChild(this.sprite),
        1 == this.ballType || (this.sprite.frame = s),
        ...
        (this.shade.width = 2.1 * t),
        (this.shade.height = 2.1 * t),
    ...
    (Ball.prototype.renderBall = function (t) {
        ...
        if (!(l > 0.499 || l < -0.499)) {
            ...
            (this.spotHolder.x = 0),
                ...
                r < Math.PI / 2 || r > (3 * Math.PI) / 2
                    ? e > Math.PI / 2 && e < (3 * Math.PI) / 2
                    ...
                    : e > Math.PI / 2 && e < (3 * Math.PI) / 2
                      ...
                      : ((this.spotHolder.y = this.circRad * Math.cos(e) * Math.sin(r)),
                        (this.spotHolder.x = this.circRad * Math.sin(e)));
            var n =
                    Math.sqrt(this.spotHolder.x * this.spotHolder.x + this.spotHolder.y * this.spotHolder.y) /
                    this.circRad,
                d = Math.cos((n * Math.PI) / 2),
                c = Math.atan2(this.spotHolder.y, this.spotHolder.x);
            (this.spotHolder.scale = new Point(1, d)),
